...
# CycloneDX document metadata for SBOMs generated by this library (UEFI §3.1.1.1).
SBOM_TOOL_VENDOR = "MatchPoint"
SBOM_TOOL_NAME = "USWID SBOM"
SBOM_DOCUMENT_AUTHORS: List[Dict[str, str]] = [
    {"name": "Richard Hughes", "email": "richard@hughsie.com"},
    {"name": "Brian Mullen", "email": "brian.desmond.mullen@gmail.com"},
]
...
class uSwidFormatCycloneDX(uSwidFormatBase):
    ...
    def save(self, container: uSwidContainer) -> bytes:
        ...
        # UEFI SBOM Guidelines §3.1.1.1: document authors (tooling) plus tag creators.
        metadata_authors: List[Dict[str, str]] = []
        seen_author_names: Set[str] = set()
        for doc_author in SBOM_DOCUMENT_AUTHORS:
            name = doc_author.get("name")
            if not name or name in seen_author_names:
                continue
            seen_author_names.add(name)
            metadata_authors.append(dict(doc_author))
        for component in container:
